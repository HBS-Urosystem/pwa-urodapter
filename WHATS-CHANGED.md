...
| Heading font                   | Source Sans 3                                            | Josefin Sans (Google Fonts); stylesheet in [`src/app.html`](src/app.html) uses **`rel="preload" as="style"`** plus **`onload`** to apply as stylesheet so the Google Fonts CSS is **not render-blocking** (better Lighthouse TBT than a synchronous `<link rel="stylesheet">`) |
