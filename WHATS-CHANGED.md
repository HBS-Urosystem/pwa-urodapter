...
| Animation  | JS image sequence (PNG frames: `intro_loop_000.png` ... `intro_loop_NNN.png`)                 | Single looping `<video>` (`intro_loop_seq.mp4`, ~212KB) with **`poster`** (`intro_loop_seq-poster.png`, first frame from scraped `intro_loop_000.png`); home [`+page.svelte`](src/routes/+page.svelte) adds **`rel="preload"`** for poster (`as="image"`) and video (`as="video"`) with **`fetchpriority="high"`** when the splash is shown so LCP resources are in the initial HTML |
