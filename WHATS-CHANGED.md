...
| `instructions-for-doctors-on-female-patients/` | `/instructions-for-doctors-on-female-patients` | Client-rendered only (`ssr = false`) so reload reapplies Instructions tab + step from `localStorage` without a “Before starting” flash; instruction steps use a **DaisyUI carousel** (full-width `carousel-item`s, horizontal scroll / swipe, snap). **Typography** matches other content pages: page title **`text-3xl`**, section titles **`text-xl font-semibold`**, body **`text-base-content/90`** at base size (no `prose-sm`); modals use **`prose prose-sm`**. Section vertical padding **`py-8`** like FAQ/contact. |
...
| Paragraph-oriented compile fields                           | N/A                                                        | Blank-line splits in **`scripts/compile-content.mjs`** (`bodyToParagraphs`) produce **`paragraphs` / `introParagraphs` / `notesParagraphs`** (and similar) for: doctor **before starting** (A/B/C), **instruction steps** (YAML `body` → `paragraphs` in `generated.ts`), **FAQ** intro and answers, **contact** company + form intro, **educational video** notes after the id line. UI uses one **`<p>`** per string (inline markdown where used). **Pre-instillation** letter sections and **legal** policy chunks keep a **single `body`** each so **`formatBlockMarkdown`** can still handle **`###`** subheads and **`-`** lists. |
