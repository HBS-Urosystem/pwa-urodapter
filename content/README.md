...
- **YAML** (`.yaml`): ordered instruction steps (`video`, `plusModalId`, multiline `body`). **`before-starting.meta.yaml`**: each `modalButtons` entry uses `placement: afterLetter` and `letter: A` | `B` | `C` so the opener sits under that point (same UI as the A-row control).
...
| `/what-to-do-prior-to-instillation`            | `pages/pre-instillation.md`                                                                      |
| `/instructions-for-doctors-on-female-patients` | `instructions/female/before-starting.md`, `before-starting.meta.yaml`, `modals.md`, `steps.yaml` |
| `/instructions-for-doctors-on-male-patients`   | `instructions/male/before-starting.md`, `before-starting.meta.yaml`, `modals.md`, `steps.yaml`   |
| `/faq`                                         | `pages/faq.md`                                                                                   |
